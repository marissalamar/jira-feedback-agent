...
st.set_page_config(
    page_title="Jira Product Intelligence", page_icon="📊", layout="wide"
)
...
with st.sidebar:
    ...
    with col_h:
        new_hour = st.selectbox(
            ...
            index=st.session_state.get("sched_hour", 2),
            key="sched_hour_sel",
        )
    with col_m:
    ...
    def update_progress(stage, page, posts_found, total_posts, current_title):
        if stage == "scraping":
            progress_bar.progress(
                5, text=f"Scraping page {page}… ({posts_found} posts in window so far)"
            )
            ...
            progress_bar.progress(
                pct,
                text=f"Analysing new post {posts_found + 1}/{total_posts} with Claude…",
            )
...
with tab_dashboard:
    ...
    with hcol:
        ...
        st.markdown(
            "<h2>Executive Summary: Jira Community Support Analysis</h2>",
            unsafe_allow_html=True,
        )
    with bcol:
    ...
    if summary:
        ...
        st.caption(
            f"Based on {good_ct} analysed posts (of {total_ct} total) · Generated {gen_time}"
        )
        st.markdown(summary["markdown"])
    else:
        st.info(
            "No summary yet — click **Refresh Data** in the sidebar to generate one."
        )
    ...
    # ── Posts table ───────────────────────────────────────────────────────────
    st.markdown(
        '<div class="section-header">Post-Level Detail</div>', unsafe_allow_html=True
    )
    ...
    with col_f2:
        ...
        sentiment_filter = st.multiselect(
            "Sentiment", options=clean_sentiments, default=clean_sentiments
        )
    with col_f3:
        theme_search = st.text_input(
            "Search theme or title", placeholder="e.g. automation"
        )
    ...
    if theme_search:
        mask = filtered["theme"].str.contains(
            theme_search, case=False, na=False
    ...
    if "post_date" in display_df.columns:
        display_df["post_date"] = display_df["post_date"].apply(
    ...
    if "sentiment" in display_df.columns:
        display_df["sentiment"] = display_df["sentiment"].map(
            lambda s: f"{sentiment_icons.get(s, '')} {s}"
        )
    ...
    # ── Severity & Sentiment ──────────────────────────────────────────────────
    st.markdown(
        '<div class="section-header">Severity & Sentiment Breakdown</div>',
        unsafe_allow_html=True,
    )
    ...
    if not high_df.empty:
        st.markdown(
            '<div class="section-header">🔴 High-Severity Issues Requiring Attention</div>',
            unsafe_allow_html=True,
        )
...
with tab_history:
    ...
    display_hist = hist_df.drop(columns=["Run"]).rename(
        columns={"Run Label": "Timestamp"}
    )
    ...
    if len(hist_df) >= 2:
        ...
        st.line_chart(
            hist_df.set_index("Run Label")[["Negative", "Neutral", "Positive"]]
        )

        st.markdown(
            '<div class="section-header">Severity Over Time</div>',
            unsafe_allow_html=True,
        )
        st.line_chart(hist_df.set_index("Run Label")[["High", "Medium", "Low"]])

        st.markdown(
            '<div class="section-header">Theme Frequency Across All Runs</div>',
            unsafe_allow_html=True,
        )
        ...
        all_theme_df = pd.DataFrame(
            all_themes.most_common(15), columns=["Theme", "Total Mentions"]
        )
        ...
        st.markdown(
            '<div class="section-header">Recurring Issues</div>', unsafe_allow_html=True
        )
        all_title_sets = [
            set(r["title"] for r in entry.get("results", [])) for entry in history
        ]
